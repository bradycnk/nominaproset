...
const AttendanceManager: React.FC = () => {
  ...
  return (
    <div className="bg-white rounded-[2rem] shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
      ...
      {activeTab === 'daily' && (
        <>
          <div className="border-b border-slate-100 bg-slate-50 px-5 py-4 sm:px-6 lg:px-8 flex flex-col gap-3">
             <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
               ...
               <div className="flex w-full gap-3 items-center md:w-auto">
                 <input
                   type="text"
                   placeholder="Buscar empleado..."
                   value={searchQuery}
                   onChange={e => setSearchQuery(e.target.value)}
                   className="flex-1 min-w-[160px] rounded-xl border border-slate-200 bg-white px-4 py-2 text-xs font-semibold text-slate-700 outline-none focus:ring-2 focus:ring-emerald-500 md:flex-none md:w-48"
                 />
                 <select
                   className="min-w-[160px] flex-1 rounded-xl border border-slate-200 bg-white px-4 py-2 text-xs font-semibold text-slate-700 outline-none focus:ring-2 focus:ring-emerald-500 cursor-pointer md:flex-none"
             ...
             {!loading && (() => {
               ...
               const completados = filtered.filter(emp => savedAttendances[emp.id]?.hora_salida).length;
               const cerrados = filtered.filter(emp => savedAttendances[emp.id]?.cerrado).length;
               return (
                 <div className="grid grid-cols-4 gap-2">
                   <div className="bg-white rounded-xl border border-slate-100 px-3 py-2 flex flex-col">
                     ...
                     <span className="text-lg font-black text-slate-700 leading-tight">{total}</span>
                   </div>
                   <div className="bg-amber-50 rounded-xl border border-amber-100 px-3 py-2 flex flex-col">
                     <span className="text-[9px] font-black uppercase tracking-widest text-amber-500">Pendientes</span>
                     <span className="text-lg font-black text-amber-600 leading-tight">{pendientes}</span>
                   </div>
                   <div className="bg-emerald-50 rounded-xl border border-emerald-100 px-3 py-2 flex flex-col">
                     <span className="text-[9px] font-black uppercase tracking-widest text-emerald-600">Laborando</span>
                     <span className="text-lg font-black text-emerald-700 leading-tight">{laborando}</span>
                   </div>
                   <div className="bg-slate-100 rounded-xl border border-slate-200 px-3 py-2 flex flex-col">
                     <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">Completados</span>
                     <span className="text-lg font-black text-slate-600 leading-tight">{completados + cerrados}</span>
                   </div>
                 </div>
               );
             })()}
                 ...
                 return (
                    <div key={title} className={`border rounded-2xl p-6 shadow-sm transition-all ${stats.isClosed ? 'bg-slate-50 border-slate-200 opacity-80' : 'bg-white border-slate-200'}`}>
                       ...
                       <button
                         ...
                         className={`w-full mt-4 py-3 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] transition-all flex items-center justify-center gap-2 ${
                            stats.isClosed
                       ...
                       >
                          {stats.isClosed ? (
                            <><span>🔒</span> Quincena Cerrada</>
                          ) : (
                            <><span>⚠️</span> Cerrar Quincena</>
                          )}
                       </button>
                    </div>
                 );
